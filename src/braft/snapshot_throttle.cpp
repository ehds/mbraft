...
BRPC_VALIDATE_GFLAG(raft_max_install_snapshot_tasks_num, brpc::PositiveInteger);
...
size_t ThroughputSnapshotThrottle::throttled_by_throughput(int64_t bytes) {
    ...
    int64_t limit_throughput_bytes_s =
        std::max(_throttle_throughput_bytes,
    ...
    if (_cur_throughput_bytes + bytes > limit_per_cycle) {
        ...
        if (now - _last_throughput_check_time_us <=
            ...
            available_size = limit_per_cycle > _cur_throughput_bytes
                                 ? limit_per_cycle - _cur_throughput_bytes
                                 : 0;
...
bool ThroughputSnapshotThrottle::add_one_more_task(bool is_leader) {
    ...
    lck.unlock();
    LOG(INFO) << "Succed to add one more task, new task num is: "
              << saved_task_num
              << ", task num threshold: " << task_num_threshold;
    return true;
}
...
void ThroughputSnapshotThrottle::finish_one_task(bool is_leader) {
    ...
    CHECK_GE(_snapshot_task_num, 0)
        << "Finishing task cause wrong task num: " << saved_task_num;
    lck.unlock();
...
void ThroughputSnapshotThrottle::return_unused_throughput(
    ...
    _cur_throughput_bytes =
        std::max(_cur_throughput_bytes - (acquired - consumed), int64_t(0));
}
