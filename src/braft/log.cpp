...
DEFINE_bool(raft_recover_log_from_corrupt, false,
            "recover log by truncating corrupted log");
...
static bvar::LatencyRecorder g_segment_append_entry_latency(
    "raft_segment_append_entry");
...
inline bool verify_checksum(int checksum_type, const char* data, size_t len,
                            uint32_t value) {
...
inline bool verify_checksum(int checksum_type, const butil::IOBuf& data,
                            uint32_t value) {
    switch (checksum_type) {
        case CHECKSUM_MURMURHASH32:
...
int Segment::_load_entry(off_t offset, EntryHeader* head, butil::IOBuf* data,
    ...
    if (data != NULL) {
        if (buf.length() < ENTRY_HEADER_SIZE + data_len) {
            ...
            const ssize_t n =
                file_pread(&buf, _fd, offset + buf.length(), to_read);
            if (n != (ssize_t)to_read) {
                return n < 0 ? -1 : 1;
            }
...
int Segment::append(const LogEntry* entry) {
    ...
    while (written < (ssize_t)to_write) {
        ...
        if (n < 0) {
            LOG(ERROR) << "Fail to write to fd=" << _fd << ", path: " << _path
                       << berror();
            return -1;
        }
        written += n;
...
int Segment::sync(bool will_sync, bool has_conf) {
    ...
    if (will_sync) {
        ...
        if (FLAGS_raft_sync_policy == RaftSyncPolicy::RAFT_SYNC_BY_BYTES &&
            FLAGS_raft_sync_per_bytes > _unsynced_bytes && !has_conf) {
            return 0;
        }
        _unsynced_bytes = 0;
...
LogEntry* Segment::get(const int64_t index) const {
    ...
    do {
        ...
        EntryHeader header;
        butil::IOBuf data;
        if (_load_entry(meta.offset, &header, &data, meta.length) != 0) {
            ok = false;
            break;
        }
        ...
        switch (header.type) {
            ...
            case ENTRY_TYPE_CONFIGURATION: {
                ...
                if (!status.ok()) {
                    LOG(WARNING)
                        << "Fail to parse ConfigurationPBMeta, path: " << _path;
                    ok = false;
                    break;
                }
            } break;
...
int Segment::close(bool will_sync) {
    ...
    if (ret == 0) {
        ...
        LOG_IF(ERROR, rc != 0) << "Fail to rename `" << old_path << "' to `"
                               << new_path << "\', " << berror();
        return rc;
    }
    return ret;
...
int Segment::truncate(const int64_t last_index_kept) {
    ...
    if (ret_off < 0) {
        PLOG(ERROR) << "Fail to lseek fd=" << _fd
...
int SegmentLogStorage::init(ConfigurationManager* configuration_manager) {
    if (FLAGS_raft_max_segment_size < 0) {
        LOG(FATAL) << "FLAGS_raft_max_segment_size "
                   << FLAGS_raft_max_segment_size
...
int SegmentLogStorage::append_entries(const std::vector<LogEntry*>& entries,
    ...
    if (_last_log_index.load(butil::memory_order_relaxed) + 1 !=
        ...
        LOG(FATAL)
            << "There's gap between appending entries and _last_log_index"
            << " path: " << _path;
        return -1;
    }
...
void SegmentLogStorage::pop_segments_from_back(
    const int64_t last_index_kept, std::vector<scoped_refptr<Segment> >* popped,
...
int SegmentLogStorage::list_segments(bool is_empty) {
    ...
    while (dir_reader.Next()) {
        ...
        if (match == 2) {
            ...
            Segment* segment =
                new Segment(_path, first_index, last_index, _checksum_type);
            _segments[first_index] = segment;
            continue;
        }

        match =
            sscanf(dir_reader.name(), BRAFT_SEGMENT_OPEN_PATTERN, &first_index);
        if (match == 1) {
...
scoped_refptr<Segment> SegmentLogStorage::open_segment() {
    scoped_refptr<Segment> prev_open_segment;
    {
        ...
        if (!_open_segment) {
            _open_segment =
                new Segment(_path, last_log_index() + 1, _checksum_type);
    ...
    do {
        if (prev_open_segment) {
            ...
            PLOG(ERROR)
                << "Fail to close old open_segment or create new open_segment"
                << " path: " << _path;
