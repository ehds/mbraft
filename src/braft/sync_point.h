...
class SyncPoint {
   public:
    ...
    // When adding a marker, it implicitly adds a dependency for the marker
    // pair.
...
#define TEST_SYNC_POINT(x) braft::SyncPoint::GetInstance()->Process(x)
#define TEST_IDX_SYNC_POINT(x, index) \
    braft::SyncPoint::GetInstance()->Process(x + std::to_string(index))
#define TEST_SYNC_POINT_CALLBACK(x, y) \
    braft::SyncPoint::GetInstance()->Process(x, y)
#define INIT_SYNC_POINT_SINGLETONS() (void)braft::SyncPoint::GetInstance();
#endif  // NDEBUG
