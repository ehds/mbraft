...
class MetaStorageManager {
   public:
    ...
    scoped_refptr<KVBasedMergedMetaStorageImpl> register_meta_storage(
    ...
    scoped_refptr<KVBasedMergedMetaStorageImpl> get_meta_storage(
        const std::string& path) {
   ...
   private:
    ...
    typedef std::map<std::string, scoped_refptr<KVBasedMergedMetaStorageImpl> >
        MetaStorageMap;
    typedef butil::DoublyBufferedData<MetaStorageMap>
        DoublyBufferedMetaStorageMap;
   ...
   public:
    ...
    void Run() {
        if (!status().ok()) {
            ...
            LOG(INFO) << "Saved merged stable meta, path " << _path << " group "
                      << _vgid << " term " << _term << " votedfor " << _votedfor
                      << " time: " << u_elapsed;
        }
...
butil::Status MixedMetaStorage::set_term_and_votedfor(
    const int64_t term, const PeerId& peer_id, const VersionedGroupId& group) {
    butil::Status status;
    if (!_is_inited) {
        LOG(WARNING) << "MixedMetaStorage not init, path: " << _path;
        status.set_error(EINVAL,
                         "MixedMetaStorage of group %s not init, path: %s",
...
// [NOTICE] Conflict cases may occur in this mode, it's important to ensure
// consistency
// 1. Single is newer than Merged:
//      case 1: upgrade storage from Single to Mixed, data in Merged is stale
//      case 2: last set_term_and_votedfor succeeded in Single but failed in
//      Merged
// 2. Merged is newer than Single:
//      case: downgrade storage from Merged to Mixed, data in Single is stale
butil::Status MixedMetaStorage::get_term_and_votedfor(
    int64_t* term, PeerId* peer_id, const VersionedGroupId& group) {
    butil::Status status;
    if (!_is_inited) {
        LOG(WARNING) << "MixedMetaStorage not init, path: " << _path;
        status.set_error(EINVAL,
    ...
    // If data from single stable storage is newer than that from merged stable
    // storage, merged stable storage should catch up the newer data to ensure
    // safety; Vice versa.
    ...
    int64_t term_2;
    PeerId peer_id_2;
    status = _merged_impl->get_term_and_votedfor(&term_2, &peer_id_2, group);
    if (!status.ok()) {
    ...
    // this case is theoretically impossible, pay much attention to it if
    // happens
    if (term_1 == term_2 && peer_id_1 != ANY_PEER && peer_id_2 != ANY_PEER) {
        CHECK(false) << "Unexpected conflict when mixed stable storage of "
                     << group << " get_term_and_votedfor, the same term "
                     << term_1 << ", but different non-empty votdfor("
                     << peer_id_1 << " from single stable storage and "
                     << peer_id_2
    ...
    if (single_newer_than_merged) {
        ...
        *peer_id = peer_id_1;
        ...
        if (!status.ok()) {
            LOG(WARNING) << "node " << group
                         << " merged stable storage failed to set term "
                         << *term << " and vote for peer " << *peer_id
        ...
        *peer_id = peer_id_2;
        ...
        if (!status.ok()) {
            LOG(WARNING) << "node " << group
                         << " single stable storage failed to set term "
                         << *term << " and vote for peer " << *peer_id
                         << " when catch up data, path " << _path
                         << ", error: " << status.error_cstr();
            return status;
...
butil::Status MixedMetaStorage::gc_instance(
    const std::string& uri, const VersionedGroupId& vgid) const {
    butil::Status status;
    std::string merged_path;
    std::string single_path;
    ...
    if (0 != gc_dir(single_path)) {
        LOG(WARNING) << "Group " << vgid << " failed to gc path "
                     << single_path;
        status.set_error(EIO, "Group %s failed to gc path %s", vgid.c_str(),
                         single_path.c_str());
        return status;
    }
    if (0 != global_mss_manager->remove_instance_from_meta_storage(merged_path,
                                                                   vgid)) {
        LOG(ERROR) << "Group " << vgid
                   << " failed to gc kv from path: " << merged_path;
        status.set_error(EIO, "Group %s failed to gc kv from path %s",
                         vgid.c_str(), merged_path.c_str());
        return status;
    }
    LOG(INFO) << "Group " << vgid
              << " succeed to gc from single path: " << single_path
              << " and merged path: " << merged_path;
    return status;
}
...
butil::Status FileBasedSingleMetaStorage::init() {
    ...
    if (!butil::CreateDirectoryAndGetError(
        ...
        status.set_error(e,
                         "Fail to create dir when init SingleMetaStorage, "
                         "path: %s",
                         _path.c_str());
        return status;
    }
    ...
    if (ret != 0) {
        ...
        status.set_error(EIO,
                         "Fail to load pb meta when init stabel storage"
                         ", path: %s",
                         _path.c_str());
        return status;
    }
...
butil::Status FileBasedSingleMetaStorage::set_term_and_votedfor(
    const int64_t term, const PeerId& peer_id, const VersionedGroupId&) {
    butil::Status status;
    if (!_is_inited) {
...
butil::Status FileBasedSingleMetaStorage::get_term_and_votedfor(
    int64_t* term, PeerId* peer_id, const VersionedGroupId& group) {
    butil::Status status;
    if (!_is_inited) {
        status.set_error(EINVAL, "SingleMetaStorage not init, path: %s",
...
int FileBasedSingleMetaStorage::load() {
    ...
    LOG(INFO) << "Loaded single stable meta, path " << _path << " term "
              << _term << " votedfor " << _votedfor.to_string()
...
int FileBasedSingleMetaStorage::save() {
    ...
    LOG(INFO) << "Saved single stable meta, path " << _path << " term " << _term
...
butil::Status FileBasedSingleMetaStorage::gc_instance(
    const std::string& uri, const VersionedGroupId& vgid) const {
    butil::Status status;
    if (0 != gc_dir(uri)) {
        LOG(WARNING) << "Group " << vgid
                     << " failed to gc single stable storage"
                        ", path: "
                     << uri;
        status.set_error(EIO,
                         "Group %s failed to gc single stable storage"
                         ", path: %s",
                         vgid.c_str(), uri.c_str());
        return status;
    }
    LOG(INFO) << "Group " << vgid
              << " succeed to gc single stable storage"
                 ", path: "
              << uri;
    return status;
}
...
butil::Status KVBasedMergedMetaStorage::init() { return _merged_impl->init(); };

butil::Status KVBasedMergedMetaStorage::set_term_and_votedfor(
    const int64_t term, const PeerId& peer_id, const VersionedGroupId& group) {
...
butil::Status KVBasedMergedMetaStorage::get_term_and_votedfor(
    int64_t* term, PeerId* peer_id, const VersionedGroupId& group) {
...
butil::Status KVBasedMergedMetaStorage::gc_instance(
    const std::string& uri, const VersionedGroupId& vgid) const {
    butil::Status status;
    if (0 != global_mss_manager->remove_instance_from_meta_storage(uri, vgid)) {
...
butil::Status KVBasedMergedMetaStorageImpl::init() {
    ...
    if (!butil::CreateDirectoryAndGetError(
        ...
        LOG(ERROR) << "Fail to create " << dir_path.value() << " : " << e;
        status.set_error(e,
                         "Fail to create dir when init MergedMetaStorage, "
                         "path: %s",
                         _path.c_str());
        return status;
    }
    ...
    if (!st.ok()) {
        lck.unlock();
        LOG(ERROR) << "Fail to open db: " << st.ToString()
                   << " path: " << _path;
    ...
    if (bthread::execution_queue_start(&_queue_id, &execq_opt,
...
void KVBasedMergedMetaStorageImpl::run_tasks(leveldb::WriteBatch& updates,
    ...
    if (!st.ok()) {
        ...
        butil::Status status;
        status.set_error(EIO,
                         "MergedMetaStorage failed to write batch"
                         ", path: %s, error: %s",
                         _path.c_str(), st.ToString().c_str());
...
void KVBasedMergedMetaStorageImpl::set_term_and_votedfor(
    ...
    if (!_is_inited) {
                                 ...
                                 "MergedMetaStorage of group %s not"
                                 " init, path: %s",
                                 group.c_str(), _path.c_str());
...
butil::Status KVBasedMergedMetaStorageImpl::get_term_and_votedfor(
    int64_t* term, PeerId* peer_id, const VersionedGroupId& group) {
    butil::Status status;
    if (!_is_inited) {
        status.set_error(EINVAL,
                         "MergedMetaStorage of group %s not init, path: %s",
                         group.c_str(), _path.c_str());
        return status;
    }
        ...
        *peer_id = ANY_PEER;
        ...
        if (!status.ok()) {
            LOG(ERROR) << "node " << group
                       << " failed to set initial term and votedfor when first "
                          "time init"
                       << ", path " << _path << ", error "
                       << status.error_cstr();
            return status;
        ...
        LOG(ERROR) << "node " << group << " failed to get value from db, path "
                   << _path << ", error " << st.ToString().c_str();
        status.set_error(EIO,
                         "MergedMetaStorage of group %s failed to"
    ...
    *term = meta.term();
    ...
    LOG(INFO) << "Loaded merged stable meta, path " << _path << " group "
              << group << " term " << *term << " votedfor " << *peer_id
              << " time: " << timer.u_elapsed();
    return status;
}
...
butil::Status KVBasedMergedMetaStorageImpl::delete_meta(
    ...
    if (!st.ok()) {
        ...
        status.set_error(EIO,
                         "MergedMetaStorage failed to delete group %s"
                         ", path: %s, error: %s",
                         group.c_str(), _path.c_str(), st.ToString().c_str());
        return status;
    }
    ...
    LOG(INFO) << "Deleted merged stable meta, path " << _path << " group "
              << group << " time: " << timer.u_elapsed();
    return status;
}
