...
class Iterator {
   ...
   public:
    ...
    void set_error_and_rollback(size_t ntail = 1,
                                const butil::Status* st = NULL);
...
class StateMachine {
   public:
    ...
    // 2. receive requests with higher term such as vote_request from a
    // candidate or append_entries_request from a new leader
...
inline const char* state2str(State state) {
    const char* str[] = {
        "LEADER", "TRANSFERRING",  "CANDIDATE", "FOLLOWER",
...
// This class encapsulates the parameter of on_start_following and
// on_stop_following interfaces.
class LeaderChangeContext {
    DISALLOW_COPY_AND_ASSIGN(LeaderChangeContext);

   public:
    LeaderChangeContext(const PeerId& leader_id, int64_t term,
    ...
    // return the information about why on_start_following or on_stop_following
    // is called.
    const butil::Status& status() const { return _st; }
...
inline std::ostream& operator<<(std::ostream& os,
                                const LeaderChangeContext& ctx) {
    os << "{ leader_id=" << ctx.leader_id() << ", term=" << ctx.term()
       << ", status=" << ctx.status() << "}";
    return os;
}
...
struct NodeStatus {
    ...
    //
    // WARNING: too many pending logs, means the processing rate can't catup
    // with the writing rate. Users can consider to slow down the writing rate
    // to avoid exhaustion of resources.
...
enum LeaseState {
    ...
    // This node is leader, but we are not sure the data is up to date. This
    // state
...
struct LeaderLeaseStatus {
    LeaderLeaseStatus() : state(LEASE_DISABLED), term(0), lease_epoch(0) {}
    ...
    // A specific term may have more than one lease, when transfer leader
    // timeout happen. Lease epoch will be guranteed to be monotinically
    // increase, in the life cycle of a node.
    int64_t lease_epoch;
};
...
struct NodeOptions {
    ...
    // The specific LogStorage implemented at the business layer, which should
    // be a valid instance, otherwise use SegmentLogStorage by default.
    //
    // Default: null
    ...
    // to avoid useless transmission. Two files in local and remote are
    // duplicate, only if they has the same filename and the same checksum
    // (stored in file meta). Default: false
    bool filter_before_copy_remote;

    // If non-null, we will pass this snapshot_file_system_adaptor to
    // SnapshotStorage Default: NULL
    ...
    //     It will never be elected as leader. So we don't need to init
    //     _vote_timer and _election_timer.
    // 2. FLAGS_raft_enable_witness_to_leader = true
    //     It can be electd as leader, but should transfer leader to normal
    //     replica as soon as possible.
    //
    // Warning:
    // 1. FLAGS_raft_enable_witness_to_leader = false
    //     When leader down and witness had newer log entry, it may cause leader
    //     election fail.
    // 2. FLAGS_raft_enable_witness_to_leader = true
    //     When leader shutdown and witness was elected as leader, if follower
    //     delay over one snapshot, it may cause data lost because witness had
    //     truncated log entry before snapshot.
    // Default: false
    bool witness = false;
...
class Node {
   public:
    ...
    //    -  Not all nodes in the raft group set |raft_enable_leader_lease| to
    //    true,
    //       and tranfer leader/vote interfaces are used;
    //    -  In the raft group, the value of |election_timeout_ms| in one node
    //    is larger
    ...
    // resource [NOTE] code after apply can't access resource in done
    ...
    // [NOTE] when list_peers concurrency with add_peer/remove_peer, maybe
    // return peers is staled. because add_peer/remove_peer immediately modify
    ...
    // Reset the |election_timeout_ms| for the very node, the
    // |max_clock_drift_ms| is also adjusted to keep the sum of
    // |election_timeout_ms| and |the max_clock_drift_ms| unchanged.
    butil::Status reset_election_timeout_ms(int election_timeout_ms);

    // Forcely reset |election_timeout_ms| and |max_clock_drift_ms|. It may
    // break leader lease safety, should be careful. Following are suggestions
    // for you to change |election_timeout_ms| safely.
    ...
    //     - Wait at least |old election_timeout_ms + new max_clock_drift_ms|
    //     times to make
    //       sure all previous elections complete.
    //     - Upgrade |election_timeout_ms| to new one, meanwhiles
    //     |max_clock_drift_ms|
    //       can set back to the old value.
    // 2. Three steps to safely upgrade |election_timeout_ms| to a smaller one:
    //     - Adjust |election_timeout_ms| and |max_clock_drift_ms| at the same
    //     time,
    //       to make the sum of |election_timeout_ms + max_clock_drift_ms|
    //       unchanged.
    //     - Wait at least |election_timeout_ms + max_clock_drift_ms| times to
    //     make
    //       sure all previous elections complete.
    ...
    // Return OK on success and user_log is assigned with the very data. Be
    // awared that the user_log may be not the exact log at the given index, but
    // the first available user log from the given index to
    // last_committed_index. Otherwise, appropriate errors are returned:
    //     - return ELOGDELETED when the log has been deleted;
    //     - return ENOMOREUSERLOG when we can't get a user log even reaching
    //     last_committed_index.
    // [NOTE] in consideration of safety, we use last_applied_index instead of
    // last_committed_index in code implementation.
    ...
    // as we see from the website.
    void get_status(NodeStatus* status);

    // Make this node enter readonly mode.
    // Readonly mode should only be used to protect the system in some extreme
    // cases. For example, in a storage system, too many write requests flood
    // into the system unexpectly, and the system is in the danger of exhaust
    // capacity. There's not enough time to add new machines, and wait for
    // capacity balance. Once many disks become full, quorum dead happen to raft
    // groups. One choice in this example is readonly mode, to let leader reject
    // new write requests, but still handle reads request, and configuration
    // changes. If a follower become readonly, the leader stop replicate new
    // logs to it. This may cause the data far behind the leader, in the case
    // that the leader is still writable. After the follower exit readonly mode,
    // the leader will resume to replicate missing logs. A leader is readonly,
    // if the node itself is readonly, or writable nodes (nodes that are not
    // marked as readonly) in the group is less than majority. Once a leader
    // become readonly, no new users logs will be acceptted.
    ...
    //      - This node is a leader, and the count of writable nodes in the
    //      group
...
struct GCOptions {
    ...
    // Version is necessary because instance with the same groupid may be
    // created again very soon after destroyed.
