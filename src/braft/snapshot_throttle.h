...
// Abstract class with the function of throttling during heavy disk
// reading/writing
class SnapshotThrottle : public butil::RefCountedThreadSafe<SnapshotThrottle> {
   public:
    ...
    // case 1: The follower and leader both try to throttle the same request,
    // and
    //         only one of them permit the request. No actual IO and bandwidth
    //         consumed, the acquired tokens are wasted.
    // case 2: We acquired some tokens, but only part of them are used, because
    // of
    //         the file reach the eof, or the file contains holes.
    virtual void return_unused_throughput(int64_t acquired, int64_t consumed,
                                          int64_t elaspe_time_us) {}
...
class ThroughputSnapshotThrottle : public SnapshotThrottle {
   public:
    ThroughputSnapshotThrottle(int64_t throttle_throughput_bytes,
                               int64_t check_cycle);
    ...
    void return_unused_throughput(int64_t acquired, int64_t consumed,
                                  int64_t elaspe_time_us);
