...
#ifndef O_CLOEXEC
#define O_CLOEXEC                                     \
    02000000 /*define close_on_exec if not defined in \
                fcntl.h*/
#endif
...
class FileAdaptor {
   public:
    virtual ~FileAdaptor() {}
    // Write to the file. Different from posix ::pwrite(), write will retry
    // automatically when occur EINTR. Return |data.size()| if successful, -1
    // otherwise.
    virtual ssize_t write(const butil::IOBuf& data, off_t offset) = 0;

    // Read from the file. Different from posix ::pread(), read will retry
    // automatically when occur EINTR. Return a non-negative integer less than
    // or equal to |size| if successful, -1 otherwise. In the case of EOF, the
    // return value is a non-negative integer less than |size|.
    virtual ssize_t read(butil::IOPortal* portal, off_t offset,
                         size_t size) = 0;
...
class FileSystemAdaptor
    : public butil::RefCountedThreadSafe<FileSystemAdaptor> {
   public:
    FileSystemAdaptor() {}
    virtual ~FileSystemAdaptor() {}

    // Open a file, oflag can be any valid combination of flags used by posix
    // ::open(), file_meta can be used to pass additinal metadata, it won't be
    // modified, and should be valid until the file is destroyed.
    virtual FileAdaptor* open(const std::string& path, int oflag,
                              const ::google::protobuf::Message* file_meta,
                              butil::File::Error* e) = 0;

    // Deletes the given path, whether it's a file or a directory. If it's a
    // directory, it's perfectly happy to delete all of the directory's
    // contents. Passing true to recursive deletes subdirectories and their
    // contents as well. Returns true if successful, false otherwise. It is
    // considered successful to attempt to delete a file that does not exist.
    ...
    virtual bool link(const std::string& old_path,
                      const std::string& new_path) = 0;

    // Creates a directory. If create_parent_directories is true, parent
    // directories will be created if not exist, otherwise, the create operation
    // will fail. Returns 'true' on successful creation, or if the directory
    // already exists.
    ...
    // Get a directory reader to read all sub entries inside a directory. It
    // will not recursively search the directory.
...
class BufferedSequentialReadFileAdaptor : public FileAdaptor {
   ...
   protected:
    BufferedSequentialReadFileAdaptor()
        : _buffer_offset(0),
          _buffer_size(0),
          _reach_file_eof(false),
          _error(0) {}

    // The |need_count| here is just a suggestion, |nread| can be larger than
    // |need_count| actually, if needed. This is useful for some special cases,
    // in which data must be read atomically. If |nread| < |need_count|, the
    // wrapper think eof is reached.
    virtual int do_read(butil::IOPortal* portal, size_t need_count,
                        size_t* nread) = 0;
...
class BufferedSequentialWriteFileAdaptor : public FileAdaptor {
   ...
   protected:
    ...
    virtual void seek(off_t offset) { _buffer_offset = offset; }
