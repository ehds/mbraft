...
bool PosixDirReader::is_valid() const { return _dir_reader.IsValid(); }
...
const char* PosixDirReader::name() const { return _dir_reader.name(); }
...
ssize_t PosixFileAdaptor::read(butil::IOPortal* portal, off_t offset,
                               size_t size) {
...
bool PosixFileAdaptor::sync() { return raft_fsync(_fd) == 0; }
...
ssize_t BufferedSequentialReadFileAdaptor::read(butil::IOPortal* portal,
    ...
    BRAFT_VLOG << "begin read offset " << offset << " count " << size
               << ", buffer_offset " << _buffer_offset << " buffer_size "
               << _buffer_size;
    if (offset < _buffer_offset ||
        offset > off_t(_buffer_offset + _buffer_size)) {
        LOG(WARNING)
            << "Fail to read from buffered file adaptor with invalid range"
            << ", buffer_offset: " << _buffer_offset
            << ", buffer_size: " << _buffer_size << ", read offset: " << offset
            << ", read size: " << size;
        errno = EINVAL;
        return -1;
    }
    if (offset > _buffer_offset) {
        _buffer.pop_front(
            std::min(size_t(offset - _buffer_offset), _buffer.size()));
...
ssize_t BufferedSequentialWriteFileAdaptor::write(const butil::IOBuf& data,
                                                  off_t offset) {
    if (_error) {
        errno = _error;
        return -1;
    ...
    BRAFT_VLOG << "begin write offset " << offset << ", data_size "
               << data.size() << ", buffer_offset " << _buffer_offset
...
FileAdaptor* PosixFileSystemAdaptor::open(
    ...
    if (e) {
        *e = (fd == -1) ? butil::File::OSErrorToFileError(errno)
                        : butil::File::FILE_OK;
    }
    if (fd == -1) {
        return NULL;
...
bool PosixFileSystemAdaptor::delete_file(const std::string& path,
                                         bool recursive) {
    butil::FilePath file_path(path);
    return butil::DeleteFile(file_path, recursive);
}

bool PosixFileSystemAdaptor::rename(const std::string& old_path,
                                    const std::string& new_path) {
    return ::rename(old_path.c_str(), new_path.c_str()) == 0;
}

bool PosixFileSystemAdaptor::link(const std::string& old_path,
                                  const std::string& new_path) {
...
bool PosixFileSystemAdaptor::create_directory(const std::string& path,
    ...
    return butil::CreateDirectoryAndGetError(dir, error,
...
bool create_sub_directory(const std::string& parent_path,
                          const std::string& sub_path, FileSystemAdaptor* fs,
                          butil::File::Error* error) {
    if (!fs) {
